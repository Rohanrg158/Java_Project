...
public class LibraryGUI extends JFrame implements IUserInterface {
    ...
    private boolean isStringEmptyOrNull(String s) { return s == null || s.trim().isEmpty(); }
    ...
    private void handleAddBook() {
        ...
        } catch (NumberFormatException ex) { showError("Book ID must be a valid number."); } 
        catch (Exception ex) { showError("Add Book Failed: " + ex.getMessage()); }
    }
    ...
    private void handleAddMember() {
        try {
            ...
            if (isStringEmptyOrNull(name)) { throw new Exception("Member name cannot be empty."); }
            if (service.isMemberIdDuplicate(id)) { throw new Exception("Member ID " + id + " already exists. Please use a unique ID."); }

            Person newMember;
            if (isStudentRadio.isSelected()) {
                String studentId = addStudentIdField.getText();
                if (isStringEmptyOrNull(studentId)) { throw new Exception("Student ID cannot be empty."); }
                newMember = new Student(id, name, studentId);
            } else { newMember = new Member(id, name); }
        ...
        } catch (NumberFormatException ex) { showError("Member ID must be a valid number."); } 
        catch (Exception ex) { showError("Add Member Failed: " + ex.getMessage()); }
    }
    ...
    private void handleBorrow() {
        ...
        } catch (NumberFormatException ex) { showError("Please enter valid numeric IDs for Member and Item."); } 
        catch (Exception ex) { showError("Borrow Failed: " + ex.getMessage()); }
    }
    ...
    private void handleReturn() {
        ...
        } catch (NumberFormatException ex) { showError("Please enter a valid numeric ID for the Item."); } 
        catch (Exception ex) { showError("Return Failed: " + ex.getMessage()); }
    }

    @Override public void displayMessage(String message) { JOptionPane.showMessageDialog(this, message, "Success", JOptionPane.INFORMATION_MESSAGE); }
    @Override public void showError(String error) { JOptionPane.showMessageDialog(this, error, "Error", JOptionPane.ERROR_MESSAGE); }
    ...
    public void refreshDataDisplay(List<LibraryItem> items, List<Person> members) {
        ...
        for (LibraryItem item : items) {
            ...
            String author = "N/A";
            if (item instanceof Book) { author = ((Book)item).getAuthor(); }
        ...
        for (Person person : members) {
            ...
            if (person instanceof Student) { details = ((Student)person).getStudentId(); }
    ...
    public static class LibraryCore {
        public static void main(String[] args) {
            SwingUtilities.invokeLater(() -> {
                try {
                    IDatabaseDAO dao = new DatabaseDAO(JDBC_URL, MYSQL_USER, MYSQL_PASS); 
